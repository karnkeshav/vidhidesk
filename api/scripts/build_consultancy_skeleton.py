"""One-off builder for templates/contracts/consultancy.docx — the docxtpl
skeleton for the Consultancy Agreement template (Sprint 2 Deliverable 2,
Batch 2).

Not run automatically; re-run manually
(`python scripts/build_consultancy_skeleton.py`) only if the skeleton's
fixed structure needs to change. Clause content lives in Postgres
(template_clauses, seeded by scripts/seed_consultancy_template.py).

Same shape as build_service_agreement_skeleton.py — no variant concept
(always asymmetric Consultant / Client), so party role labels are
hardcoded literal text here, not the `{{ party_a_role_label }}`
mechanism NDA uses.
"""
...
OUTPUT_PATH = (
    Path(__file__).resolve().parent.parent.parent / "templates" / "contracts" / "consultancy.docx"
)
...
def build() -> None:
    ...
    title_run = title.add_run("CONSULTANCY AGREEMENT")
    title_run.bold = True
    ...
    preamble.add_run(
        "This Consultancy Agreement (“Agreement”) is made and entered into as "
        "of {{ effective_date }} (“Effective Date”) by and between:"
    )
    ...
    party_a.add_run(
        ...
        "referred to as the “Consultant”, which expression shall, unless "
    ...
    sig_a.add_run("For and on behalf of {{ party_a_name }} (Consultant):").bold = True
